...
export default function BotDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchRunner = async () => {
      ...
      const [healthResult, perfResult, tradesResult, proposalsResult, connectorResult] = await Promise.all([
        ...
        getRunnerBotTrades(baseUrl, id, 20),
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-8">
                <h3 className="text-2xl font-bold mb-4 text-white">OpenClaw Integration 🦞</h3>
  ...
  -d '{"connectorType":"openclaw","mode":"shadow","version":"0.1.0"}'`}</code>
              ...
              <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-8">
                <h3 className="text-2xl font-bold mb-2 text-white">Trade History</h3>
                <p className="text-xs text-white/40 mb-4">Showing latest 20 trades</p>
                {runnerStatus === 'unconfigured' && (
                  <p className="text-white/40 text-sm">Runner not configured.</p>
                )}
                {runnerStatus === 'loading' && (
                ...
                {runnerStatus === 'online' && runnerTrades.length > 0 && (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {runnerTrades.slice(0, 20).map((trade) => (
